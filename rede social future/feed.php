...
echo '<section class="menu">';
   if (isset($_POST["descricao"])) {
                ...
                $offseta = $offseta-($offseta%$limit); 

                $meusgrupos = $db->query("select grupo.nome, grupo.codigo from grupo join grupo_usuario on grupo_usuario.grupo = grupo.codigo join usuario on usuario.email = grupo_usuario.usuario where usuario.email = '" . $login . "' and grupo_usuario.ativo = 1 order by grupo.".$orderbya." limit ".$limit." offset ".$offseta."");
                ...
                for ($page = 0; $page < ceil($total/$limit); $page++) {
                    echo (($offseta == $page*$limit) ? ($page+1) : "<a href=\"".urla("offseta", $page*$limit, $login)."\">".($page+1)."</a>")." \n";
                }
                ...
                if (isset($arraypesquisa) && $arraypesquisa != null) {
                    ...
                    for ($page = 0; $page < ceil($newtotal/$limit); $page++) {
                        echo (($ofset == $page*$limit) ? ($page+1) : "<a href=\"".urla("ofset", $page*$limit, $login, $campo)."\">".($page+1)."</a>")." \n";
                    }
                }
...
//TOP ASSUNTOS!!!
$orderbyassuntos = (isset($_GET["orderbyassunto"])) ? $_GET["orderbyassunto"] : "codigo asc";
$whereassuntos = array();
...
echo "<tr>\n";
echo "<td><b>Mês</b><td>\n";
echo "<td><b>Assunto</b> <a href=\"".urlassunto("orderbyassunto", "assunto+asc")."\">&#x25BE;</a> <a href=\"".urlassunto("orderbyassunto", "assunto+desc")."\">&#x25B4;</a></td>\n";
echo "<td></td>\n";
echo "</tr>\n";
    ...
    foreach ($comments as $info)
    {
        ...
        {         
            ...
            if ($login == $info["email"])
            {
                ...
                echo '<a href=update_dados.php?email='.$login.'&codigo='.$info["codigo"].' style = "float: right; margin-right: 10px" color: #fff >Editar</a>';
                echo '</form>';
...
(select nome
    ...
    echo "<td><b>Sugestões de amizade</b> <a href=\"".urlassunto("orderbyassunto", "sugestao+asc")."\">&#x25BE;</a> <a href=\"".urlassunto("orderbyassunto", "sugestao+desc")."\">&#x25B4;</a></td>\n";
    echo "</tr>\n";
...
if (isset($_POST['compartilhar']))
{
 ...
 if (isset($_POST['delete'])){
                ...
                if (isset($_POST['adicionar'])){
                  $total2 = $db->query("select count(codigo) as total2 from assunto")->fetchArray() ["total2"];
                  $total2++;
