...
echo '<tr>';
echo '<td>';
echo '<input type="text" id="email" name="email" placeholder="E-mail" required>';
echo '</td>';
echo '</tr>';
echo '<tr>';
echo '<td>';
...
echo '</select>';
echo '<input type="text" id="outro" name="outro" placeholder="Insira o nome do país" style="display: none;" required disabled>';
echo '<select id="uf" name="uf" onchange="muda();">';
echo '</select>';
echo '<input type="text" id="outra" name="outra"placeholder="Insira o nome da unidade federativa" style="display: none;" required disabled>';
echo '<select id="cidade" name="cidade" onchange="mostra();">';
echo '</select>';
echo '<input type="text" id="outra1" name="outra1" placeholder="Insira o nome da cidade" style="display: none;" required disabled>';
echo '</td>';
echo '</tr>';
echo '<tr>';
...
$results = $db->query("select pais.codigo as pais_cod, pais.nome as pais_nome from pais where ativo = 1");
...
$results = $db->query("select codigo, nome, pais from uf where ativo = 1");

while ($row = $results->fetchArray()) {
    echo "uf_row.push('".$row['codigo']."".'*'."".$row['nome']."".'*'."".$row['pais']."');\n";
}

$results = $db->query("select codigo, nome, uf from cidade where ativo = 1");
...
if (isset($_POST['email'])) {
        ...
        if ($result === null) {
            if ($_POST['pais'] == 'outro') {
                $result = querySingle("select codigo from pais where nome = '".transform($_POST['outro'])."'");
                if ($result === null) {
                    $db->exec("insert into pais (nome) values ('".transform($_POST['outro'])."')");
                }
                if ($_POST['uf'] == 'outra') {
                    $result = querySingle("select codigo from uf where nome = '".transform($_POST['outra'])."'");
                    if ($result === null) {
                    ...
                    if ($_POST['cidade'] == 'outra1') {
                        $result = querySingle("select codigo from cidade where nome = '".transform($_POST['outra1'])."'");
                        if ($result === null) {
                            ...
                            echo "<script> setTimeout(function () { window.open(\"feed.php?email=".strtoupper($_POST['email'])."\",\"_self\"); }, 3000); </script>";
                        }
                    } else {
                        $db->exec("insert into usuario (email, nome, data_nascimento, genero, cidade) values ('".strtoupper($_POST['email'])."', '".strtoupper($_POST['nome'])."', '".$_POST['data_nascimento']."', '".$_POST['genero']."', ".$_POST['cidade'].")");
                        echo "<script> setTimeout(function () { window.open(\"feed.php?email=".strtoupper($_POST['email'])."\",\"_self\"); }, 3000); </script>";
                    }
                } else {
                    if ($_POST['cidade'] == 'outra1') {
