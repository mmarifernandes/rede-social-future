...
for ($c = 0;$c < count($verifUsuarios);$c++)
{
    ...
    if ($result != 0)
    {
        $result = $db->querySingle("select interacao.hora_post from usuario join interacao on usuario.email = interacao.usuario where usuario.email = '" . $verifUsuarios[$c] . "' limit 1");
        if ($result !== null)
        {
