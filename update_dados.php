...
$results = $db->query("select usuario, hora_post as data, tipo, conteudo from interacao where codigo = ".$_GET["codigo"]);
...
$results7 = $db->query("select grupo.nome as nome, grupo.codigo as codigo from grupo_usuario join grupo on grupo_usuario.grupo = grupo.codigo where grupo_usuario.ativo = 1 and usuario = '".$login."'");
while ($row7 = $results7->fetchArray())
{
    echo "<option value=\"" . $row7["codigo"] . "\">" . ucwords(strtolower($row7["nome"])) . "</option>\n";
}
echo '</select>';
echo '</div>';
    }else{
