...
h3 span {
    ...
    echo "<form name=\"login\" class=\"login\" action=\"feed.php\" method=\"post\" onsubmit=\"return confirma();\">\n";

    echo '<h3>Bem-vindo ao <span>FUTURE</span></h3>';
    echo '<h4>Entre na sua conta.</h4>';
    echo '<br>';
	echo '<select name="selectuser" id="selectuser" autofocus required>';
	$results = $db->query("select usuario.email as email, usuario.nome as nome, usuario.genero as genero from usuario");
    echo '<option value="" disabled selected>Selecione um usuário</option>';
	while ($row = $results->fetchArray()) {
		echo "<option value=\"".$row["email"]."\">".ucwords(strtolower($row["nome"])).' - '.strtolower($row["email"])."</option>\n";
	}

    echo '</select>';
    echo '<button class="button login__submit">';
