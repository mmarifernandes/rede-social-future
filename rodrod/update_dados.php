...
if (isset($_GET['email'])) {
    if(isset($_GET['atributo']) && $_GET['atributo'] == 'nome') {
        ...
        echo '<td><input type="text" id="nome1" name="nome1" disabled="true" value="'.ucwords(strtolower($result)).'"></td>';
        echo '</tr>';
        echo '<br>';
        echo '<tr>';
    ...
    if(isset($_GET['atributo']) && $_GET['atributo'] == 'data_nascimento') {
        ...
        echo '<td><input type="text" id="data_nascimento1" name="data_nascimento1" disabled="true" value="'.strftime('%d/%m/%Y', strtotime($result)).'"></td>';
        echo '</tr>';
        echo '<br>';
        echo '<tr>';
    ...
    if(isset($_GET['atributo']) && $_GET['atributo'] == 'genero') {
        ...
        echo '<td><input type="text" id="genero1" name="genero1" disabled="true" value="'.$result.'"></td>';
        echo '</tr>';
        echo '<br>';
        echo '<tr>';
    ...
    if(isset($_GET['atributo']) && $_GET['atributo'] == 'localidade') {
        ...
        echo '<td><input type="text" id="pais1" name="pais1" disabled="true" value="'.ucwords(strtolower($row['pais'])).'"></td>';
        echo '</tr>';
        echo '<tr>';
        echo '<td><label for="uf1">Unidade federativa atual</label></td>';
        echo '<td><input type="text" id="uf1" name="uf1" disabled="true" value="'.ucwords(strtolower($row['uf'])).'"></td>';
        echo '</tr>';
        echo '<tr>';
        echo '<td><label for="cidade1">Cidade atual</label></td>';
        echo '<td><input type="text" id="cidade1" name="cidade1" disabled="true" value="'.ucwords(strtolower($row['cidade'])).'"></td>';
        echo '</tr>';
        echo '<tr>';
        echo '<td><br></td>';
        echo '</tr>';
...
if (isset($_POST['pais'])) {
    if ($_POST['pais'] == 'outro') {
        $db->exec("insert into pais (codigo, nome) values ('".str_replace(' ', '_', transform($_POST['outro']))."', '".strtoupper($_POST['outro'])."')");
        if ($_POST['uf'] == 'outra') {
            $db->exec("insert into uf (codigo, nome, pais) values ('".str_replace(' ', '_', transform($_POST['outra']))."', '".strtoupper($_POST['outra'])."', '".str_replace(' ', '_', transform($_POST['outro']))."')");
            if ($_POST['cidade'] == 'outra1') {
                ...
                $db->exec("insert into cidade (codigo, nome, uf) values (".$ultCidade.", '".strtoupper($_POST['outra1'])."', '".str_replace(' ', '_', transform($_POST['outra']))."')");
                $db->exec("update usuario set cidade = ".$ultCidade." where email = '".$_GET['email']."'");
                echo "<script> setTimeout(function () { window.open(\"dados_usuario.php\",\"_self\"); }, 3000); </script>";
            } else {
                $result = $db->querySingle("select codigo from cidade where nome = '".strtoupper($_POST['cidade'])."'");
                $db->exec("update usuario set cidade = ".$result." where email = '".$_GET['email']."'");
                echo "<script> setTimeout(function () { window.open(\"dados_usuario.php?email=".$_GET['email']."\",\"_self\"); }, 3000); </script>";
            }
        } else {
            if ($_POST['cidade'] == 'outra1') {
        ...
        if ($_POST['uf'] == 'outra') {
            $db->exec("insert into uf (codigo, nome, pais) values ('".str_replace(' ', '_', transform($_POST['outra']))."', '".strtoupper($_POST['outra'])."', '".strtoupper($_POST['pais'])."')");
            if ($_POST['cidade'] == 'outra1') {
                $ultCidade = $db->querySingle("select codigo from cidade group by 1 order by 1 desc limit 1");
                $ultCidade = $ultCidade + 1;
                $db->exec("insert into cidade (codigo, nome, uf) values (".$ultCidade.", '".strtoupper($_POST['outra1'])."', '".str_replace(' ', '_', transform($_POST['outra']))."')");
